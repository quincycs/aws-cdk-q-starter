...
class AssetPublishing extends CoreConstruct {
  ...
  constructor(scope: Construct, id: string, private readonly props: AssetPublishingProps) {
    super(scope, id);
    this.myCxAsmRoot = path.resolve(this.assemblyBuilderOf(this.appOf(this)).outdir);
  ...
  private assemblyBuilderOf(stage: Stage): cxapi.CloudAssemblyBuilder {
    return (stage as any)._assemblyBuilder;
  }

  private appOf(construct: IConstruct): App {
